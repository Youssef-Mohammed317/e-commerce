...
<section class="max-w-4xl md:w-[90%] p-6 mx-auto bg-white rounded-md shadow-md dark:bg-gray-800">
  <form enctype="multipart/form-data" action="items.php?from=<?php echo $from ?>&do=edit&id=<?php echo $item['item_id']; ?>" method="POST" class="item-fields">
    <h2 class="mb-5 text-center text-2xl/9 font-bold tracking-tight text-blue-700 dark:text-gray-200">Item</h2>
    ...
    <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
              ...
              <div class="col-span-2">
              <select id="member" name="member" required  class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 dark:focus:border-blue-300 focus:outline-none focus:ring">
                <option value="" disabled>Select The Member</option>
                <?php
                  foreach ($members as $member) {
                    $check = $item['member'] == $member['id']?'selected': '';
              ...
              <select id="category"  required  name="category" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 dark:focus:border-blue-300 focus:outline-none focus:ring">
                ...
                <?php
                  foreach ($categories as $category) {
                    $check = $item['category'] == $category['id']?'selected': '';
        ...
        <div class="py-7">
          <span id="change-image" data-required onclick="document.getElementById('edit-image').click()" class="cursor-pointer px-8 py-2.5 leading-5 text-white transition-colors duration-300 transform bg-blue-700 rounded-md hover:bg-blue-600 focus:outline-none focus:bg-blue-600">Change Image</span>
          <input name="image"   id="edit-image" class="hidden w-full mb-5 text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" id="default_size" type="file">
        </div>
    ...
    <section class="item-preview max-w-4xl md:w-[90%] p-6 mx-auto flex justify-center items-center bg-white rounded-md shadow-md dark:bg-gray-800">
      <div class="w-full mx-auto flex flex-col justify-between max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        <div>
          <a href="#">
            <img id="image-field" class="p-8 rounded-t-lg w-full" src="<?php echo $item['image']==''?'../assets/imgs/product-image.png':$item['image'] ?>" alt="product image" />
          </a>
        </div>
